...
- **Rule G — a reveal is knowledge about the answer, never a re-score of a guess (R-036).** It fixes a letter to a position in the *solution*. It does not touch any row: a row records what a guess scored, and a row is read against the word that was typed. Under Mirror a reveal describes **one** solution and is marked with which; it is never shown against the other (§7.2).
...
**R-029 · Silent Start withholds a yellow; it does not invent a grey.** Raised from playtest immediately after R-028: *"how then does Silent Start work, it's very confusing since my keyboard keys are disabled."* The suppression step reported `GREY` in place of `YELLOW`, and grey does not mean "no information" — it means **not in this word**. So a letter that was present read as absent, greyed on the keyboard, and stayed wrong for the rest of the word with nothing to correct it. That is a lie rather than a withholding, and it is also why R-014 had to exist at all: combined with `RL.02` The Sieve, the false grey could lock a letter the answer needed.
→ **Ruled:** suppressed tiles report `UNKNOWN`, the state that already means "no reading here" and ranks below `GREY` in the keyboard derivation, so the key is left untouched instead of marked absent. Genuinely absent letters still report `GREY` — the modifier denies you the yellow signal, not the board's ability to say what is simply not there. The Sieve interaction is now structurally impossible rather than guarded after the fact.

**R-030 · A staked word must say so.** `word_challenge` was written to run state by an event and **never read**: `EV.01` The Wager put a relic on a two-guess solve and then neither branch fired. A wager that cannot be lost is not a wager, and one that cannot be won is worse. Nothing rendered it either, so a player had no way to know a stake was live.
→ **Ruled:** the challenge resolves at word end, in `finishWord`, against `guessesUsed` — success and failure branches both run through the same event-effect vocabulary that set them. It renders as its own banner above the modifiers, in the red rather than the amber, counting down. A modifier says how a word is harder; a challenge says what you have staked on it, and reading them as one strip buries the one that can cost a relic.

**R-031 · An activation that produces nothing is refused, not consumed.** Raised from playtest against `RL.07` The Auditor: *"it's kind of buggy, sometimes it activates sometimes not."* The engine charged the declared cost **before** running the handler, so naming a letter the Auditor cannot answer — one already tried — took 5g and returned in silence: no stamp, no error, and nothing to distinguish it from a use that worked. §6.6's rule 2 already says an unaffordable activation is refused rather than consumed; this is the same principle for a different cause, and it was missing because the affordability check happens before the cost and the *usefulness* check happens after it.
→ **Ruled:** if the handler yields no effects, the whole use is refused — no gold, no guess, no burnt cap — and the player is told. Enforced in the engine so no relic has to remember it. Two supporting fixes: a `UNTRIED_LETTER` picker no longer offers letters already tried, since the input kind says which letters are legal and offering the rest invites the refusal; and `USE_ITEM` emitted `CONSUMABLE_USED` for relics as well as consumables, which is what the relic chips read to know they fired — relics now emit `ACTIVATION_FIRED`.

**R-032 · Bought information must be visible after the drawer closes.** Also from playtest: *"when I go from the relic screen it doesn't activate at all."* The Auditor's verdict was written only into `word.revealed.letters`, where the keyboard rendered it as a faint tint on one key. So a player paid 5g, the drawer closed over the answer, and the result was a slightly different shade — indistinguishable from nothing happening.
→ **Ruled:** letters bought with gold or a guess render as an explicit stamped row on the word screen, held for the whole word, because that is how long the fact stays true. Information the player paid for should be the most legible thing on the board rather than the least.

**R-033 · A boss's rule belongs where rules live, not in a marquee.** Raised from playtest — *"what is that upper row that appeared, I'm confused"* — against a scrolling red ticker at the Cipher. Three faults at once. It was a **third** row saying "boss", above a `BOSS` badge and a node line already saying `THE CIPHER`. A marquee is clipped at both edges, so the only statement of what the boss actually does read as broken text: `IO ANSWER UNTIL THE THIRD`. And bosses carry no modifiers, so the banner beside the badge rendered as an empty amber bar — the one place a rule would have been readable was blank.
→ **Ruled:** a boss's rule is part of its definition in `bosses.ts` and renders as the note in the modifier banner, static, beside the badge. The ticker is no longer mounted during a word; it is kept for the S.09 intro screen that `BOSS_INTRO` is declared for and nothing enters yet, where a marquee is fine because atmosphere is that screen's only job. The ticker also read `BOSSES[state.actIndex]` and held its own copy of each boss's rule as a conditional — both are the drift R-026 was written for, and both now come from the node and the definition.
