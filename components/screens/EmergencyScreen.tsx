...
/**
 * The emergency purchase. MECHANICS.md §2.4: the offer is MANDATORY, not
 * optional UI — the player must always see the exit they did or did not buy.
 *
 * R-012 is the reason this screen exists as its own step rather than a toast on
 * the death screen: the design's death mock showed the resource exhausted next
 * to 145g unspent, which would have covered all three purchases. That is what
 * an unexercised branch looks like.
 */
export function EmergencyScreen({ state }: { state: GameState }) {
  ...
  const step = state.emergencyPurchases + 1;
    ...
    <div className="flex flex-1 flex-col justify-center gap-4 px-4 py-6">
      <div className="border border-red bg-panel p-4">
        <p className="font-mono text-[9px] leading-none tracking-[0.2em] text-red">
          BANKROLL AT ZERO · WORD UNSOLVED
        </p>
        ...
        <p className="mt-3 font-mono text-[11px] leading-[1.6] text-fg2">
          Purchase {step} of {CONFIG.economy.emergencyCosts.length} this run. The price
          escalates and never resets — there is no fourth.
        </p>
      ...
      <div className="flex flex-col gap-2">
        <Button primary={affordable} disabled={!affordable} onClick={() => dispatch({ type: 'BUY_EMERGENCY' })}>
          {affordable ? `BUY ${CONFIG.economy.emergencyGrant} GUESSES` : 'CANNOT AFFORD IT'}
        </Button>
        <button
          type="button"
