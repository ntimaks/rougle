...
**§13 lists thirty-two engineering problems.** Fourteen came from reading the specs, six (I-15 … I-20) from building Phase 1, two (I-21, I-22) from measuring it, two (I-23, I-24) from the content the specs never contained, one (I-25) from measuring R-025, one (I-26) from playtesting the result one (I-27) from a sweep that turned out to be measuring the bot one (I-28) from a flag that was accepted and ignored and one (I-29) from a metric that indicted the wrong file, one (I-30) from an A/B whose two arms were the same config, one (I-31) from a sweep that returned identical rows, and one (I-32) from R-044 naming a gap in its own coverage. **Twenty-six are now ruled** — see MECHANICS.md §11 R-014 … R-036 — and the rest are marked with what they block. Read §13 before starting a ticket.
...
**I-30 · RESOLVED. The solver's narrowing cache made a run depend on the runs before it.** `filterCandidates` prefix-caches each row's narrowing in a process-lifetime `Map`, and the key named the bot's vocabulary by its **word count**: `${length}:${pool.length}:${solutionIndex}`. `vocabularyGap` hands different runs different ~958-word slices of the same list, and slices of equal size are common, so one run would narrow against another run's vocabulary. Silent while the cache only ever grew — an ambiguous key is at least *consistently* wrong — and observable the moment `NARROW_CACHE_LIMIT` (20k entries, about 150 runs) cleared it, after which a lookup that used to hit now missed and computed a different answer. Two passes of an **identical** config over 400 seeds then disagreed on 346 of them, and an economy A/B where both arms were the same config by value read four points of win rate apart. Every sweep of 200+ runs since the cache landed was measuring the order its configs happened to execute; every sweep under ~100 was fine, which is why the calibration (`cleanBaseline`, well under the limit) came back byte-identical and the existing Gate 1 test at 25 runs passed throughout. The key also omitted `silentStart` and `rangefinder`, both of which change how a row narrows. → **Fixed**: a `WeakMap` serial number per distinct vocabulary array (`poolId`) plus both flags in the key. `opener` had the same shape of bug — cached by length while taking the caller's candidate set, so every run played the opener the process's *first* run derived — and no measurable effect, because the best opener is the same word over almost any 90% slice; it now takes no pool at all. Re-measured: the §2.2 pool derivation survives unchanged (same ordering, same choice, ±2 points) and Gate 3 still passes. The standing rule: **a cache that outlives a run is part of the run's inputs, so its key must name everything its value depends on** — and the test for that is not "does the sweep repeat" but "does clearing the cache change any answer", which fails immediately at any size. `resetSolverCaches()` exists for that test.

**I-31 · RESOLVED. Three MECHANICS numbers were unreachable by the harness.** `config.ts` promises that the harness overrides `CONFIG` wholesale and that nothing else hardcodes a number appearing in MECHANICS.md. Three did: `BOSSES` is frozen at import against the default `CONFIG`, so `gauntlet.pool`, `gauntlet.words` and `cipherDeferralDepth` reached the engine as constants, and `applyRefund` read `CONFIG.gauntlet.pool` for its ceiling while `projectBoard` read `CONFIG.decayTurns`. The visible symptom was a sweep of the Gauntlet pool from 14 down to 8 returning six **byte-identical** rows — which reads exactly like "not a difficulty lever", the same conclusion §2.2 had already reached honestly about Act III's pool, and would have been filed as one. It is in fact the strongest lever in the game (MECHANICS.md §2.2). → **Fixed**: `bossFor(actIndex, cfg)` supplies the config-derived numbers, `applyRefund` and `projectBoard` take `cfg`. `BOSSES` stays exported for the UI and the report, which only ever run on the default config. The standing rule: **a sweep that returns identical rows has two explanations, and "not a lever" is the second one to check.**
...
**I-32 · R-044 priced six relics and named the other sixteen unpriceable.** `relics.json`'s `_notes.value_budget` and MECHANICS.md §11 R-044 both say it plainly: only the relics that move bankroll or payout directly (`RL.11`, `RL.12`, `RL.13`, `RL.15`, `RL.19`, and `RL.31`'s payout half) carry a measured `value_note`, because `sim/v2/relicvalue.ts` and `sim/v2/bleed.ts` can price a payout bonus with arithmetic alone — it is a function of `basePayout` and Clamp A/B, both already in `lib/engine/economy/bankroll.ts`. The other sixteen change how many guesses a word takes or how much gold moves, and neither number is arithmetic: a guess saved depends on what the solver's candidate set looks like with the relic's info applied, and a gold delta depends on when the relic's hook fires across a run. All sixteen are fully implemented (`lib/engine/content/impl/*.ts`; `PENDING_IMPLEMENTATION` is empty) and reachable through `resolveHook` — the gap is entirely on the pricing side, not the content side. The table below is that scoping: what each relic actually changes, and the specific instrumentation `sim/` is missing to measure it. Four recurring gaps cover all sixteen:

- **Guesses-saved-per-word, isolated.** `sim/v2/relicvalue.ts`'s `guessDistribution` and `sim/baseline.ts`'s `solveClean` already drive the real solver through `filterCandidates`, but always with `revealed`, `presetTiles` and `locked` hardcoded empty — there is no isolated-loadout entry point that equips exactly one relic's hook effects (via the same `projectBoard`/`resolveHook` pipeline `runner.ts`'s `buildView` already uses) and reports mean guesses against `cleanBaseline`. This is the missing piece for every INFO relic in the table.
- **Scaling relics need a run, not a word.** Three of the sixteen (`RL.03`, `RL.23`, `RL.26`) carry a counter that grows across a run, so an isolated single-word measurement understates them. The guesses-saved tool above needs a multi-word mode that plays an ordered sequence and lets the counter accumulate, the way `playRun` already does for a full board.
- **Gold/bankroll-flow-per-relic.** Every `BANKROLL`/`GOLD` effect already carries a `reason` string that is the relic's code (`lib/engine/core/effects.ts`, surfaced on `BANKROLL_CHANGED`/`GOLD_CHANGED` events in `lib/engine/core/bank.ts` and `lib/engine/economy/bankroll.ts`) — but `GameState.stats.goldEarned`/`goldSpent` (`lib/engine/core/state.ts`) only sum everything into one running total, discarding it. The fix is bucketing those events by `reason` per run instead of summing them flat, then converting the per-relic gold total to a bankroll-equivalent at the same exchange rate the refill ladder already fixes (R-043: ~27g/bankroll at the first rung), so a gold relic's value lands in the bankroll/word unit the §6.5 ceiling uses.
- **Activation policy.** `I-22` already flags this: the solver holds `RL.07`, `RL.20`, `RL.21`, `RL.28` and never uses them, so none of the four can be measured until the solver decides *when* to spend the ability, not just whether to hold it.

Two of the sixteen need something neither bucket covers: `RL.06` doesn't touch guesses or gold at all — `playRun`'s `MAP` case always walks `map.available[0]` (runner.ts:165), so route choice is unmodeled and Cartographer's entire effect is invisible to the harness. `RL.09` and `RL.30` move bankroll directly but as a rare floor/safety-net event, not a per-word rate — pricing them needs a paired same-seed A/B (relic vs no relic) on win rate and death point, not a guesses- or gold-per-word figure. → **Not ruled — scoping only.** This turns "sixteen relics, unpriced" into four buildable tools; a follow-up ticket should build the guesses-saved isolated-loadout harness first, since eleven of the sixteen depend on it either directly or through the activation-policy prerequisite.

| Code | Name (rarity) | What it does | Implemented | Sim tooling needed |
|---|---|---|---|---|
| `RL.01` | LEXICON (COMMON) | `onWordStart`: reveals the solution's vowel count; MK.II adds the repeat-letter flag. | Yes | Guesses-saved-per-word, isolated: set `view.revealed.vowelCount`/`hasRepeat` from the relic alone and compare mean guesses to `cleanBaseline`. |
| `RL.02` | THE SIEVE (COMMON) | `onFeedbackTransform`: grey letters lock off the keyboard for the word; MK.II extends the lock to the whole act. | Yes | Guesses-saved-per-word, isolated, with `view.locked` populated by the relic alone; MK.II needs the multi-word/run mode since the lock persists across words. |
| `RL.03` | PALIMPSEST (UNCOMMON) | `onWordStart`, scaling (`SOLUTIONS REMEMBERED`): reveals a letter shared with a remembered solution; hit rate rises with words solved. | Yes | Guesses-saved-per-word, isolated **and** run-scoped — a single-word test can't see the counter grow, so this needs the multi-word mode played across an ordered sequence. |
| `RL.04` | RANGEFINDER (RARE) | `onFeedbackTransform`: yellow tiles report distance instead of letter identity; MK.II adds direction. | Yes | Smallest lift of the sixteen — `sim/solver.ts`'s `consistent()`/`packObserved` already model distance constraints. Just needs the isolated-loadout wiring to equip only `RL.04` and measure mean guesses. |
| `RL.06` | CARTOGRAPHER (UNCOMMON) | `onNodeEnter`: reveals every node's modifiers (MK.II: + shop rarity) before the player picks a path. | Yes | Doesn't fit "guesses or gold" at all — it's a route-choice relic and `playRun`'s `MAP` case always takes `map.available[0]` (runner.ts:165), so no route decision exists to measure. Needs a route-choice policy in the runner first, then a route-quality metric (e.g., elite/modifier avoidance rate). |
| `RL.07` | THE AUDITOR (RARE) | `onGuessSubmit`, activation, 15g/use: names an untried letter, stamped PRESENT/ABSENT. | Yes | Needs an activation policy (I-22) deciding when to spend the 15g, **plus** per-relic gold-spent tracking **and** guesses-saved-per-word, netted into one bankroll/word figure via the gold↔bankroll exchange rate. |
| `RL.09` | THE ANVIL (UNCOMMON) | `onRunStart`/`onNodeEnter`: bankroll starts 2 lower; Forge nodes give 2 operations instead of 1 (MK.II refunds the 2 on first Forge use). | Yes | Its bankroll effects are already `reason`-tagged (`'RL.09'`/`'RL.09 MK.II'`) in the effect stream — needs `stats` to bucket by `reason` instead of summing flat, plus a metric for what the second Forge op is worth (only fires if a Forge is visited with something upgradeable offered). |
| `RL.14` | THE GUILLOTINE (UNCOMMON) | `onWordSolved`, scaling (`HEADS TAKEN`): pays escalating gold on a ≤2-guess solve, takes 30g on a ≥5-guess solve. | Yes | Gold-flow-per-relic tracking (bucket `GOLD_CHANGED` by `reason='RL.14'` across a run), run-scoped for the counter, converted to bankroll-equivalent. |
| `RL.20` | BLINDFOLD (UNCOMMON) | `onGuessSubmit`, activation: skip feedback on a guess, gain 4 bankroll if it was within Hamming distance 1 (MK.II: ≤2). | Yes | Needs an activation policy (I-22), a tracked distribution of "P(guess within Hamming distance *k* of the solution)" (the proximity analogue of `relicvalue.ts`'s `fastShare`), and accounting for the future-guess cost of forgoing that turn's feedback. |
| `RL.21` | ALL IN (RARE) | `onWordStart`, activation: wager N guesses, get them back on solving within N or lose them all (Clamp A). | Yes | Needs a wager-sizing policy for the solver (I-22), a tracked P(solve within wagered count) distribution, and per-relic bankroll-delta tracking under Clamp A. `balance_flag` also asks for its RL.04/RL.07 co-occurrence to be measured, which needs the same activation policy. |
| `RL.22` | POLYGLOT (BOSS) | `onWordSolved`: doubles gold on 6-/7-letter words; the next shop stocks one rarity tier higher. | Yes | Gold-flow-per-relic tracking for the doubled portion. The shop-tier bump is a second, harder gap with no arithmetic equivalent at all — it needs a way to price "the next shelf is one tier richer," which isn't a guess or gold number. |
| `RL.23` | THE TIN CUP (UNCOMMON) | `onGuessSubmit`, scaling (`RATE`): every guess spent yields gold, rate rising by 1 (MK.II: 2) each slow (≥4-guess) solve. | Yes | Gold-flow-per-relic tracking, run-scoped for the escalating counter, converted to bankroll-equivalent. |
| `RL.26` | THE LANTERN (RARE) | `onGuessSubmit`, scaling (`TIMES LIT`): lights letters after failed guesses, lighting one more every third time (MK.II: lights sooner). | Yes | Guesses-saved-per-word, isolated **and** run-scoped, same shape as `RL.03`. |
| `RL.28` | SHAVED COIN (RARE, Act III+) | `onFeedbackTransform`, activation: re-rolls which positions report truthfully, once a word (MK.II twice), unseen by the player. | Yes | Guesses-saved-per-word, isolated, **plus** it only matters on Liar-Letter words (I-05) so `guessesPerWord` needs to be broken out by modifier, not just by act as `RunResult.guessesPerWordByAct` does today. Also needs the activation policy (I-22). |
| `RL.29` | THE MASK (BOSS) | `onFeedbackTransform`: full immunity to Liar Letter (MK.II: + Fog). | Yes | Same as `RL.28` — guesses-saved-per-word measured only on the corrupted-modifier words it protects, which needs guesses-per-word broken out by modifier. |
| `RL.30` | OUROBOROS (BOSS) | `onBankrollChange`: the first time bankroll would hit zero, it returns to 8 instead (MK.II: 12), once a run. | Yes | Not a per-word rate at all — it's a survival relic. Needs a paired same-seed A/B (with vs without) on win rate / death point, the same shape `balance_flag` asks for against `RL.20`/`RL.21`. |
